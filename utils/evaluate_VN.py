...
def validate_data(gtFilePath, submFilePath):
    ...
    if len(submJson) != len(gtJson['data']) : 
        ...
        print("The Det file is not valid (invalid number of answers. Expected:", str(len(gtJson['data'])), " Found:", str(len(submJson)), ")")
    
    gtQuestions = sorted([r['questionId'] for r in gtJson['data']])
    res_id_to_index = {r['questionId']: ix for ix, r in enumerate(submJson)}
    ...
    if( (gtQuestions == detQuestions) == False ):
        ...
        print("The Det file is not valid. Question IDs must much GT")
    ...
    for gtObject in gtJson['data']:
        try:
            ...
            print("The Det file is not valid. Question ", str(gtObject['questionId']), " not present")
...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
    ...
    for gtObject in gtJson['data']:
        if len(gtObject['answers']) == 1 and gtObject['answers'][0] == "":
            print(gtObject)
            continue
        q_id = gtObject['questionId'];
        if not q_id in res_id_to_index.keys(): 
            continue
        res_ix = res_id_to_index[q_id];
        detObject = submJson[res_ix];
        acc = 0
        ...
        if q_id in question_ids_to_exclude:
            ...
            for answer in gtObject['answers']:
                ...
                current_acc = accuracy(gt_answer, det_answer)
                if current_acc == 1:
                    print("gt_answer: ", gt_answer)
                    print("det_answer: ", det_answer)
                    print("-"*30)
                    acc = 1
...
def display_results(results, show_answer_types):
    ...
    print("\nOverall Acc: {:2.4f}".format(results['result']['acc']))
...
python utils/evaluate_VN.py \
    ...
    --submission_file submission/ViInfographicVQA/submission_M4C_val_v0_VN.json \
    --output result_evaluate
    
python utils/evaluate_VN.py \
    --ground_truth submission/ViInfographicVQA/gt/VietInfographicVQA_test_v1.0_converted.json \
    --submission_file submission/ViInfographicVQA/submission_M4C_test_v0_VN.json \
    --output result_evaluate
